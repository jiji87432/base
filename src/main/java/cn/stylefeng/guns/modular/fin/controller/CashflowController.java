...
public class CashflowController extends BaseController {
    ...
    @RequestMapping("/list")
    @ResponseBody
    public Object list(@RequestParam(required = false) String condition,@RequestParam(required = false)String flowType) {
        //根据条件查询
        Page<Map<String, Object>> result = cashflowService.selectByCondition(condition,flowType);
